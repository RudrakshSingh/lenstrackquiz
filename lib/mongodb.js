...
if (!uri) {
  ...
  // In production mode, use a module-scoped variable to reuse connection
...
export async function getDatabase(dbName = 'lensquiz') {
  try {
    ...
    if (!client || !client.topology || !client.topology.isConnected()) {
      console.warn('MongoDB client not connected, attempting to reconnect...');
      // Reset clientPromise to allow reconnection
      if (process.env.NODE_ENV === 'production') {
        clientPromise = null;
        const newClient = new MongoClient(process.env.MONGODB_URI, options);
        clientPromise = newClient.connect();
        const newClientInstance = await clientPromise;
        return newClientInstance.db(dbName);
      }
    }
    ...
    if (error.message && (error.message.includes('MONGODB_URI') || error.message.includes('not configured'))) {
      throw new Error('MongoDB is not configured. Please add MONGODB_URI to environment variables.');
    }
