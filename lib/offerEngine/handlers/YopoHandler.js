...
export class YopoHandler {
  ...
  execute(cart, rule, state) {
    ...
    // YOPO only applies to FRAME + LENS combos
    const frameType = frame.type || 'FRAME';
    if (frameType !== 'FRAME') {
      return null; // YOPO does not apply to sunglasses, contact lenses, or accessories
    }
    ...
    // Tag free item (V1.0 Spec requirement)
    let freeItem = null;
    if (frameMRP >= lensPrice) {
      // Frame is higher, lens is free
      freeItem = {
        type: 'LENS',
        value: lensPrice,
        description: `Free Lens (₹${lensPrice})`
      };
    } else {
      // Lens is higher, frame is free
      freeItem = {
        type: 'FRAME',
        value: frameMRP,
        description: `Free Frame (₹${frameMRP})`
      };
    }
    ...
    return {
      ...
      freeItem: freeItem // V1.0 Spec: Tag free item
    };
  }
