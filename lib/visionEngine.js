...
/**
 * Get required index based on SE and frame type
 */
export function getRequiredIndexByPower(se) {
  const absSE = Math.abs(se);
  
  if (absSE <= 2) return 1.50;
  if (absSE <= 4) return 1.56;
  if (absSE <= 6) return 1.60;
  if (absSE <= 8) return 1.67;
  return 1.74; // > 8D
}

/**
 * Get required index based on frame type and SE
 */
export function getRequiredIndexByFrame(frameType, se) {
  ...
  switch (frameType) {
    ...
    case 'full_rim_metal':
      ...
      if (absSE <= 4) return 1.60;
      if (absSE <= 6) return 1.67;
      return 1.74;
      
    case 'half_rim':
    case 'semi_rimless':
      if (absSE <= 2) return 1.60;
      if (absSE <= 4) return 1.60;
      if (absSE <= 6) return 1.67;
      if (absSE <= 8) return 1.74;
      return null; // Not recommended > 8D
    ...
    case 'drilled':
      ...
      if (absSE <= 4) return 1.60;
      if (absSE <= 6) return 1.67;
      if (absSE <= 7) return 1.74;
      return null; // BLOCKED > 7D
